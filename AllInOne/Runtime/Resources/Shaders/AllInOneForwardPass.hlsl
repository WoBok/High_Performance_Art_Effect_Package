#ifndef ALL_IN_ONE_PASS_INCLUDED
#define ALL_IN_ONE_PASS_INCLUDED
...
void LitPassFragmentSimple(
    ...
    InitializeSimpleLitSurfaceData(input.uv,
    #ifdef _DISSOLUTIONSWITCH_ON
        input.dissolutionUV,
    #endif
    surfaceData);
    ...
    color.a = OutputAlpha(color.a, IsSurfaceTypeTransparent(_Surface));
        ...
        factor = step(_OutlineWidth, saturate(factor));
        color.rgb *= factor;
    #endif

    #ifdef _FRESNELSWITCH_ON
        half3 fresnel = pow((1 - saturate(dot(input.normalWS, inputData.viewDirectionWS))), _FresnelPower) * _FresnelColor.rgb;
        color.rgb += fresnel;
    #endif

    #ifdef _HSISWITCH_ON
        color.rgb *= _Brightness;
        half gray = dot(color.rgb, half3(0.2125, 0.7154, 0.0721));
