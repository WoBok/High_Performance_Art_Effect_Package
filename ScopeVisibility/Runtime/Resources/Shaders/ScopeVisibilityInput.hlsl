...
float3 _Center;
float _Angle;
//x,y: Center(x,z), z: Radius, w: Angle
float4 _VisibilityParam;
CBUFFER_END
...
float ComputeAlpha_Square(float3 positionWS) {
    float x = abs(positionWS.x - _VisibilityParam.x);
    float zDistance = positionWS.z - _VisibilityParam.y;
    x -= zDistance * _VisibilityParam.w;
    return 1 - step(_VisibilityParam.z, x);
}

float ComputeAlpha_Circle(float3 positionWS) {
    float dis = length(positionWS.xz - _VisibilityParam.xy);
    return 1 - step(_VisibilityParam.z, dis);
}
...
out SurfaceData outSurfaceData) {
    ...
    #if defined _VISIBILITY_SQUARE
        outSurfaceData.alpha = ComputeAlpha_Square(positionWS);
    #elif defined  _VISIBILITY_CIRCLE
        outSurfaceData.alpha = ComputeAlpha_Circle(positionWS);
    #else
